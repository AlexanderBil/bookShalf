          ...
          <form className = "col s12">
            <div className="input-field col s6">
              {(bookDirty && bookError) &&  <div style={{color: 'red'}}>{bookError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue={name} name = "name" id="book-name" type="text" className="validate" required/>
                 <label className="active" htmlFor="first_name2">Book</label>
            </div>

            <div className="input-field col s6">
            {(authorDirty && authorError) &&  <div style={{color: 'red'}}>{authorError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue="" name = "author" id="book-author" type="text" className="validate" required/>
