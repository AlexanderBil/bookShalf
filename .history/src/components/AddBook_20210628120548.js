          ...
          <form className = "col s12">
            <div className="input-field col s6">
              {(bookDirty && bookError) &&  <div style={{color: 'red'}}>{bookError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue={book} name = "name" id="book-name" type="text" className="validate" required/>
                 <label className="active" htmlFor="first_name2">Book</label>
            </div>

            <div className="input-field col s6">
            {(authorDirty && authorError) &&  <div style={{color: 'red'}}>{authorError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue={author} name = "author" id="book-author" type="text" className="validate" required/>
                 <label className="active" htmlFor="first_name2">Author</label>
            </div>

            <div className="input-field col s6">
            {(categoryDirty && categoryError) &&  <div style={{color: 'red'}}>{categoryError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue={category} name = "category" id="book-category" type="text" className="validate" required/>
                 <label className="active" htmlFor="first_name2">Category</label>
            </div>

            <div className="input-field col s6">
            {(isbnDirty && isbnError) &&  <div style={{color: 'red'}}>{isbnError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => handleSubmit(e)} defaultValue={isbn} name = "isbn" id="book-isbn" type="text" className="validate" required/>
