...
const bookHandler = (e) => {
  ...
  if(e.target.value == ''){
    setBookError('Мalue is not correct ')
  }else{
    setBookError('')
  }
}
          ...
          <form className = "col s12">
            <div className="input-field col s6">
              {(bookDirty && bookError) &&  <div style={{color: 'red'}}>{bookError}</div>}
               <input onBlur = {(e) => blurHandler(e)} onChange = {(e) => {handleSubmit(e); bookHandler()}} value={book} name = "name" id="book-name" type="text" className="validate" required/>
