...
// функція для створення редагування (по кліку на Edit заносим в стейт booksEdit книгу яка спывпадаэ з id )
  const editItem = (e) => {
    fetch('./db.json')
    .then(data => data.json())
  ...
  // функція яка обнуляєж стейт booksEdit при натисненні кнопки Back компонента Edit
  const deletetItem = () => {
    setBooksEdit({})
    console.log(1);
  }
